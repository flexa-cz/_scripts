 ...
 * @author https://github.com/solarissmoke/php-moon-phase
 **/
...
class MoonPhaseForDay extends MoonPhase{
	...
	public function __construct($pdate=null)
	{
		parent::__construct($pdate);
		$this->curent_date=date('Y-m-d', $pdate);
	}

	public function moon_phase_for_day(){
		$begin_date=$this->curent_date.' 00:00:00';
		$begin_time=strtotime($begin_date);
		$begin_moon = new MoonPhase($begin_time);
		$begin_percent=round(($begin_moon->illumination())*100,2);
		$phase = $begin_moon->phase() < 0.5 ? 'Waxing' : 'Waning';
		$stage = $begin_moon->phase() < 0.5 ? 'First' : 'Last';

		$end_date=$this->curent_date.' 23:59:59';
		$end_time=strtotime($end_date);
		$end_moon = new MoonPhase($end_time);
		$end_percent=round(($end_moon->illumination())*100,2);

		if($begin_moon->phase()>0.5 && $end_moon->phase()<0.5){$moon_phase_for_day='New Moon';}
		elseif($begin_time<$this->next_fulL_moon() && $this->next_fulL_moon()<$end_time){$moon_phase_for_day='Full Moon';}
		...
		return $moon_phase_for_day;
	}

	public function next_fulL_moon(){
		$half_synmonth=($this->synmonth*24*60*60)/2;
		$today=strtotime($this->curent_date.' 00:00:00');
		if($today<$next_full_moon=round($this->next_new_moon()-$half_synmonth,4)){
			return $next_full_moon;
		}
		return round($this->next_new_moon()+$half_synmonth,4);
	}
}